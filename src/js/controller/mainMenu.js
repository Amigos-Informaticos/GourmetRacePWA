async function getCategories() {
  ...
  categories.forEach((category) => {
    ...
    $template
      .querySelector("a")
      .setAttribute("onclick", `getRestaurants(${category.id_category})`);
...
async function getRestaurants(category = "", name = "") {
  ...
  if (response.status === 200) {
    ...
    restaurants.forEach((restaurant) => {
      $template
        .querySelector(".box")
        .setAttribute("id", `box-${restaurant.id_restaurant}`);
      $template.querySelector(".box").dataset.id_restaurant =
      ...
      $template.querySelector(".restaurant-name").dataset.id_restaurant =
        restaurant.id_restaurant;
      $template
        .querySelector(".restaurant-location")
        .setAttribute("href", `${restaurant.location}`);
      $template
        .querySelector(".restaurant-location")
        .setAttribute("target", "_blank");
      $template.querySelector(".restaurant-location").textContent = "Ubicación";
      $template.querySelector(".restaurant-price").dataset.id_restaurant =
        restaurant.id_restaurant;
      $template.querySelector(".restaurant-price").dataset.price =
        restaurant.price;
        ...
        restaurant.schedules.forEach((objDay) => {
          ...
          if (isOpen) {
            $template.querySelector(
              ".restaurant-schedule"
            ).textContent = `${objDay.startTime} - ${objDay.endTime}`;
          } else {
      ...
      $template.querySelector(".rating").dataset.id_restaurant =
        restaurant.id_restaurant;
      $template.querySelector(".rating").dataset.score = restaurant.score;
      $template.querySelector(".box-text-container").dataset.id_restaurant =
        restaurant.id_restaurant;
